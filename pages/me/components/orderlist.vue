			...
			<ul class="carlist-all">
				<li class="carlist-order" v-for="shopItem of shoplist">
					...
					<view class="carlist-order-adress">
						<view class=""><text class="address-name">{{shopItem.urser}}</text><text class="address-num">{{shopItem.number}}</text></view>
						<view class="address-di">{{shopItem.address}}</view>
					</view>
					<view class="carlist-order-foot">
						<button size="mini" class="button-delea" @click="deleatShoplist">前去支付</button>
						<view class="goods-prize">总价:<text class="prize-prize-all">131</text></view>
					</view>
				</li>
			</ul>
		</form>
