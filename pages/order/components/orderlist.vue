<template>
				...
				<li class="carlist-order" v-for="item,index of shoplist">
					...
					<view class="carlist-order-pro">
						...
						<view class="carlist-ny">
							...
							<view class="pro-size">{{item.prosize}} x<text class="pro-num">{{item.num}}</text></view>
							<view class="pro-prize">￥{{item.prize}}元</view>
					...
					<view class="carlist-order-foot">
						...
						<view>共<text class="carlist-order-num">{{item.num}}</text>件</view>
						<view class="goods-prize">合计:<text class="prize-prize-all">{{item.prize *item.num}}</text>元</view>
					</view>
					<view class="carlist-order-bz">
	...
	<!-- 			<label class="radio-all" >
				...
				<button form-type="submit" size="mini" class="submit-btn">去支付</button>
			</view>
...
<style scoped>
	...
	.carlist-order-foot view:nth-child(1) {
		font-size: 24rpx;
	}
	.carlist-order-foot text{
		color: #FF465C;
	}
	.carlist-order-bz{
		margin-top: 10rpx;
